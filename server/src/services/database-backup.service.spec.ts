...
describe(DatabaseBackupService.name, () => {
  ...
  describe('S3 backup support', () => {
    beforeEach(() => {
      StorageCore.setMediaLocation('s3://immich-test-bucket/media-prefix');
    ...
    it('should create database backups in s3', async () => {
      const done = vitest.fn().mockResolvedValue(undefined);
      const stream = new PassThrough();

      vitest.spyOn(S3AppStorageBackend.prototype, 'writeStream').mockResolvedValue({ stream, done });
      const copyObject = vitest.spyOn(S3AppStorageBackend.prototype, 'copyObject').mockResolvedValue();
      const deleteObject = vitest.spyOn(S3AppStorageBackend.prototype, 'deleteObject').mockResolvedValue();
      mocks.process.spawnDuplexStream.mockImplementation(() => mockDuplex()('command', 0, 'data', ''));

      await sut.createDatabaseBackup();
    ...
    it('should upload backup files to s3', async () => {
      const putObject = vitest.spyOn(S3AppStorageBackend.prototype, 'putObject').mockResolvedValue();

      await sut.uploadBackup({ originalname: 'path.sql.gz', buffer: Buffer.from('buffer') } as never);

      expect(mocks.storage.createOrOverwriteFile).not.toHaveBeenCalled();
      expect(putObject).toHaveBeenCalledWith('s3://immich-test-bucket/media-prefix/backups/uploaded-path.sql.gz', expect.any(Buffer));
    });

    it('should list backups from s3', async () => {
      vitest
        .spyOn(S3AppStorageBackend.prototype, 'list')
        .mockResolvedValue(['immich-db-backup-20250729T110116-v1.234.5-pg14.5.sql.gz']);
    ...
    it('should delete backup files from s3', async () => {
      const deleteObject = vitest.spyOn(S3AppStorageBackend.prototype, 'deleteObject').mockResolvedValue();

      await sut.deleteBackup(['filename.sql']);

      expect(mocks.storage.unlink).not.toHaveBeenCalled();
      expect(deleteObject).toHaveBeenCalledWith('s3://immich-test-bucket/media-prefix/backups/filename.sql');
    });

    it('should return s3 backup path for downloads', () => {
      expect(sut.downloadBackup('hello.sql.gz')).toEqual(
        expect.objectContaining({
          path: 's3://immich-test-bucket/media-prefix/backups/hello.sql.gz',
        }),
      );
    });
  });
