...
export class DatabaseBackupService {
  ...
  private getS3(): S3AppStorageBackend | null {
    ...
    const s3c = storage?.s3;
    if (storage?.engine === 's3' && s3c?.bucket) {
  ...
  private isS3Path(path: string, s3: S3AppStorageBackend | null): boolean {
    return path.startsWith('s3://') || (!!s3 && StorageCore.isImmichPath(path));
  }

  private requireS3(s3: S3AppStorageBackend | null): S3AppStorageBackend {
    if (!s3) {
  ...
  private async createBackupReadStream(filePath: string, s3: S3AppStorageBackend | null): Promise<Readable> {
    if (this.isS3Path(filePath, s3)) {
      return this.requireS3(s3).readStream(filePath);
    }

    return this.storageRepository.createPlainReadStream(filePath);
  }

  private async assertBackupExists(filePath: string, s3: S3AppStorageBackend | null): Promise<void> {
    if (this.isS3Path(filePath, s3)) {
      await this.requireS3(s3).head(filePath);
      return;
    }

    await this.storageRepository.stat(filePath);
  }
  ...
  async createDatabaseBackup(filenamePrefix: string = ''): Promise<string> {
    ...
    try {
      ...
      if (this.isS3Path(temporaryFilePath, s3)) {
        const s3Storage = this.requireS3(s3);
        const { stream, done } = await s3Storage.writeStream(temporaryFilePath);
        await pipeline(pgdump, gzip, stream);
        await done();
        await s3Storage.copyObject(temporaryFilePath, backupFilePath);
        await s3Storage.deleteObject(temporaryFilePath);
      } else {
      ...
      if (this.isS3Path(temporaryFilePath, s3)) {
        await this.requireS3(s3)
          .deleteObject(temporaryFilePath)
          .catch((error) => this.logger.error(`Failed to delete failed backup object: ${error}`));
      } else {
  ...
  async uploadBackup(file: Express.Multer.File): Promise<void> {
    ...
    const filePath = this.joinPaths(backupsFolder, `uploaded-${fn}`);
    const s3 = this.getS3();
    if (this.isS3Path(filePath, s3)) {
      await this.requireS3(s3).putObject(filePath, file.buffer);
      return;
    }
  ...
  downloadBackup(fileName: string): ImmichFileResponse {
    ...
    const filePath = this.joinPaths(StorageCore.getBaseFolder(StorageFolder.Backups), fileName);
  ...
  async deleteBackup(files: string[]): Promise<void> {
    ...
    await Promise.all(
      files.map((filename) => {
        const filePath = this.joinPaths(backupsFolder, filename);
        if (this.isS3Path(filePath, s3)) {
          return this.requireS3(s3).deleteObject(filePath);
        }

        return this.storageRepository.unlink(filePath);
      }),
    );
  }
  ...
  async cleanupDatabaseBackups() {
    ...
    for (const file of toDelete) {
      ...
      if (this.isS3Path(filePath, s3)) {
        await this.requireS3(s3).deleteObject(filePath);
      } else {
  ...
  async restoreDatabaseBackup(
    ...
    try {
      ...
      const s3 = this.getS3();
      const backupFilePath = this.joinPaths(StorageCore.getBaseFolder(StorageFolder.Backups), filename);
      await this.assertBackupExists(backupFilePath, s3); // => check file exists
      ...
      if (backupFilePath.endsWith('.gz')) {
        const fileStream = await this.createBackupReadStream(backupFilePath, s3);
        const gunzip = this.storageRepository.createGunzip();
        fileStream.pipe(gunzip);
        inputStream = gunzip;
      } else {
        inputStream = await this.createBackupReadStream(backupFilePath, s3);
      }
      ...
      try {
        ...
        const fileStream = await this.createBackupReadStream(restorePointFilePath, s3);
        const gunzip = this.storageRepository.createGunzip();
        fileStream.pipe(gunzip);
        inputStream = gunzip;
