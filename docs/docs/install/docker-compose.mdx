...
:::tip S3/MinIO Storage (optional)
Immich can store media in S3‑compatible object storage instead of the local `/data` volume. See Administration → S3/MinIO Storage for configuration, migration using `aws s3 sync`, and caveats.

[Read the S3/MinIO guide](/docs/administration/s3-storage)
:::
